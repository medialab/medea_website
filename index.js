...
function parseGoogleDocument(result) {
  ...
  result.subtitle = drive.utils.formatAuthorName(drive.utils.clean($('.subtitle').html()));
  ...
  $('h1').each( function(i, el){
    ...
    var contents = $(this).nextUntil('h1').filter(function(d) {
      ...
      if (html.match(/##\d+##/) !== null) {
        ...
        html = html.replace(/##\d+##/g, '<a href="#" classToKeep="exponent" popover-trigger="focus" popover-append-to-body="true" popover-placement="right" popover="{{section.notes[' + (indexNote - 1 )+ ']}}" noteIndex="' + indexNote + '">' + '\\*' + '</a>');
      }
