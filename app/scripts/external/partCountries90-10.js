(function(undefined) {
  ...
  Histogram.prototype.drawChart = function(container, params) {
    var height = params.height || this.defaultHeight,
        ...
        title = (params.title || 'noTitle').replace(/ /g, ''),
    ...
    for (var i = 0; i < dataArray.length; i++) {
      if (!indexFound && sumAtIndex[i] >= 0.9 * sum) {
        indexFound = true;
        indexToSplitAt = i;
        concatResult = {
          x: (indexToSplitAt+1),
    ...
    chart.append('text')
      ...
      .attr('y', chartTrueHeight + margin.top + 10 * tickSize)
      .attr('id', 'xAxisLegend')
      .style('text-anchor', 'middle')
      .style('alignment-baseline', 'hanging')
      .style('dominant-baseline', 'hanging')
  ...
  if (typeof exports !== 'undefined') {
    ...
    exports.countryPart9010 = Histogram;
  } else if (typeof define === 'function' && define.amd)
    define('countryPart9010', [], function() {
      return Histogram;
    });
  else
    this.countryPart9010 = Histogram;
}).call(this);
