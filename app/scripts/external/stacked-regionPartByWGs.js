(function(undefined) {
  ...
  StackedBarsRegionPartByWGs.prototype.load = function(path, callback) {
    var self = this;
  ...
  StackedBarsRegionPartByWGs.prototype.drawViz = function(container, params) {
  ...
  StackedBarsRegionPartByWGs.prototype.updateData = function(container, params) {
    var height = params.height || this.defaultHeight,
        width = params.width || this.defaultWidth,
        margin = params.margin || this.defaultMargin,
    ...
    var effHeight = height - (margin.top + margin.bottom),
        ...
        scale = d3.scale.linear()
                    .range([0, barHeight])
                    .domain([0, d3.max(Object.keys(data).map(function(e) {
                      return (+data[e].total);
                    }))]),
          ...
          .attr('transform',function(d,i) {
            ...
            .data(function(d) {
              return Object.keys(data[d].regions).sort(function(a, b) {
                ...
                console.log('i', i, data[d].regions[e])
                var object = {
                      regionSymbol: e,
                ...
                if (i === 0)
                  yPositions = [effHeight - scale(data[d].regions[a[i]].participation)];
                else {
                  yPositions.push(yPositions[i-1] -
                    scale(data[d].regions[a[i]].participation));
                }
      ...
      d3.select(container + ' svg')
        ...
        .call(d3.svg.axis()
              .scale(
                d3.scale.linear().range([barHeight, 0]).domain([0, d3.max(Object.keys(data).map(function(e) {
                      return (+data[e].total);
                    }))])
                )
              .ticks(1)
              .tickValues([0, d3.max(Object.keys(data).map(function(e) {
                      return (+data[e].total);
                    }))])
  ...
  if (typeof exports !== 'undefined') {
    if (typeof module !== 'undefined' && module.exports)
      exports = module.exports = StackedBarsRegionPartByWGs;
    exports.StackedBarsRegionPartByWGs = StackedBarsRegionPartByWGs;
  } else if (typeof define === 'function' && define.amd)
    define('StackedBarsRegionPartByWGs', [], function() {
      return StackedBarsRegionPartByWGs;
    });
  else
    this.StackedBarsRegionPartByWGs = StackedBarsRegionPartByWGs;
